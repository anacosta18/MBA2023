...
## ------------------------------ CARREGANDO O DB------------------------------
mdndc <- read.csv(file = "C:/Users/anabr/Downloads/20230723_MasterDE7.csv", header = TRUE, sep = ";", dec = ",")
...
panel_2<- mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= pop_pop_million, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
  labs(x ="Cluster" , y = "População (em milhões)")

panel_3 <- mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= pop_density, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_4<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= pop_sex_ratio, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_5<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= export_dolar, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw()+
  labs(x ="Cluster" , y = "Exportação (bilhões de dólares)")

panel_6<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= import_dolar, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_7<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= gpd_dolar, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_8<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= gpd_perc,fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_9<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= FDI_WDI/1000000000, fill = cluster_K,colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_10<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= co2_eq_WDI/1000,fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_11<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= co2percapita,fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
...
panel_12<-mdndc %>% group_by(cluster_K) %>%
  ggplot(aes(x=cluster_K, y= co2pergpd, fill = cluster_K, colour = cluster_K))+
  geom_boxplot()+
  theme_bw() +
